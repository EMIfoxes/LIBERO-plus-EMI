...
suit_name = 'libero_10_no_noops'
data_dir = '/home/lxx/project/LIBERO-plus/datasets/rlds/libero_plus_data_4suite'
outpu_dir = '/home/lxx/project/DreamVLA/datasets/Dream-adapter_datasets_10'
dataset_name = 'libero_spatial'
os.makedirs(Path(outpu_dir)/'episodes', exist_ok=True) # 创建文件夹

builder = tfds.builder(suit_name, data_dir=data_dir)
...
for episode in tqdm(islice(ds_episode, num_examples), total=num_examples):
    ...
    episode_path = Path(outpu_dir)/'episodes'/str(episode_index).zfill(6)  # 创建episodes文件夹
    ...
    for steps_index,steps in enumerate(episode['steps']):  
        ...
        with h5py.File(f'{steps_path}/data.h5', 'w') as h5_file: # 创建 data.h5 文件
            ...
            # dataset_name
            h5_file.create_dataset(name='dataset_name', data=np.array(dataset_name, dtype=h5py.string_dtype(encoding='utf-8')))
...
with h5py.File(f'{str(outpu_dir)}/episodes_info.h5', 'w') as h5_file:
    episodes_dir = Path(outpu_dir)/'episodes'
